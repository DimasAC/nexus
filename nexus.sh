...
# Prompt user for Prover ID (already there, no change)
read -p "Enter your Prover ID: " PROVER_ID

# Update Prover ID to XPXGXOSL
PROVER_ID="XPXGXOSL"  # Your actual Prover ID
...
# Add Prover ID to the command
SERVICE_CMD="$HOME/.cargo/bin/cargo run --release --bin prover -- --prover-id $PROVER_ID beta.orchestrator.nexus.xyz"
